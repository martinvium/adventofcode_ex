defmodule Aoc.Year2018.Day15.BeverageBandits do
  ...
  *27730*`. 
  ...
  Outcome: 20 * 937 = *18740*`*What is the outcome* of the combat described in your puzzle input? 


  """

  defmodule Goblin do
    defstruct(power: 3, hitpoints: 200)
  end
  ...
  def part_1(input) do
    ...
    board = initial |> Enum.map(fn row -> Enum.map(row, &to_board/1) end)

    units =
      initial
      |> to_units()

    game_loop(%{board: board, units: units})
  end
  ...
  def to_units(initial) do
    ...
    |> Enum.reduce([], fn {row, y}, acc ->
      new_row =
        row
        |> Enum.with_index()
        |> Enum.map(fn {grapheme, x} -> {grapheme, x, y} end)
        |> Enum.map(&to_unit/1)
        |> Enum.reverse()
