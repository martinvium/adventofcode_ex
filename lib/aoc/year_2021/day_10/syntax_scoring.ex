defmodule Aoc.Year2021.Day10.SyntaxScoring do
  @moduledoc """
  ...
  `Syntax error in navigation subsystem on line: all of them`*All of them?!* The damage is worse than you thought. You bring up a copy of the
  ...
  *close* with one of four legal pairs of matching characters: 
  ...
  subsystem. *What is the total syntax error score for those erro
  ## --- Part Two --- 
  ...
  def part_1(input) do
    ...
    |> Enum.map(fn {invalid, _} -> invalid end)
    |> Enum.reject(&is_nil/1)
    |> Enum.map(fn invalid -> @scores[invalid] end)
    |> Enum.sum()
  end
  ...
  defp parse(line, opened) do
    next = hd(line)

    cond do
      @pairs |> Map.has_key?(next) -> parse(tl(line), [next | opened])
      next == @pairs[hd(opened)] -> parse(tl(line), tl(opened))
      true -> {next, opened}
    end
  end
end
