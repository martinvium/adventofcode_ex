defmodule Aoc.Year2021.Day08.SevenSegmentSearch do
  ...
  `  0:      1:      2:      3:      4:
   *aaaa*    ....    *aaaa    aaaa*    ....
  *b    c*  .    *c*  .    *c*  .    *c  b    c**b    c*  .    *c*  .    *c*  .    *c  b    c*
   ....    ....    *dddd    dddd    dddd**e    f*  .    *f  e*    .  .    *f*  .    *f**e    f*  .    *f  e*    .  .    *f*  .    *f**gggg*    ....    *gggg    gggg*    ....

    5:      6:      7:      8:      9:
   *aaaa    aaaa    aaaa    aaaa    aaaa**b*    .  *b*    .  .    *c  b    c  b    c**b*    .  *b*    .  .    *c  b    c  b    c**dddd    dddd*    ....    *dddd    dddd*
  .    *f  e    f*  .    *f  e    f*  .    *f*
  .    *f  e    f*  .    *f  e    f*  .    *f**gggg    gggg*    ....    *gggg    gggg*`So, to render a `1`, only segments `c` and `f` would be turned on; the rest
  ...
  *all ten unique signal patterns* you see, and then write down a single *four
  ...
  cdfeb fcadb cdfeb cdbaf`(The entry is wrapped here to two lines so it fits; in your notes, it will all
  be on a single line.) 
  ...
  *In the output values, how many times do digits `1`, `4`, `7`, or `8` appe
  ## --- Part Two --- 
  ...
  def part_1(input) do
    ...
    |> Enum.map(fn signal -> String.length(signal) end)
    |> Enum.frequencies()
    |> Map.take([2, 3, 4, 7])
