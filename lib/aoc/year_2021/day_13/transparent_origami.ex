defmodule Aoc.Year2021.Day13.TransparentOrigami do
  ...
  *How many dots are visible after completing just the first fold instruction on
  ...
  """
  def part_1(input) do
    {dots, _folds} = input |> prepare_input()
    grid = prepare_grid(dots)
    grid |> List.flatten() |> length() |> IO.inspect()
  end
  ...
  defp prepare_input(input) do
    [dots_input, folds_input] = input |> String.split("\n\n")
    ...
    folds =
      ...
      |> String.split("\n")
      |> Enum.map(fn row ->
        [_, _, instruction] = row |> String.split(" ")
        [axis, value] = instruction |> String.split("=")
        {axis, value |> String.to_integer()}
      end)

    {dots, folds}
  end

  defp prepare_grid(dots) do
    _width = dots |> Enum.map(fn [x, _] -> x end) |> Enum.max()
    _height = dots |> Enum.map(fn [_, y] -> y end) |> Enum.max()

    # for y <- 1..height do
    #   for x <- 1..width do
    #     # dots |> Enum.member?({x, y})
    #     false
    #   end
    # end
  end
