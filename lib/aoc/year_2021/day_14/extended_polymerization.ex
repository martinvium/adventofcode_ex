defmodule Aoc.Year2021.Day14.ExtendedPolymerization do
  ...
  def calc(input, count) do
    ...
    template = a |> String.graphemes()

    insertions =
      b
      |> String.split("\n")
      |> Enum.map(fn row ->
        [left, right] = row |> String.split(" -> ")
        [left |> String.graphemes(), right]
      end)

    frequencies =
      step(template, insertions, count)
      |> Enum.frequencies()
      |> Enum.map(fn {_, value} -> value end)
      |> Enum.sort()

    least = frequencies |> hd()
    most = frequencies |> Enum.reverse() |> hd()

    most - least
  end

  def step(template, _, 0), do: template

  def step(template, insertions, count) do
    new_template =
      template
      ...
      |> Enum.reduce([], fn
        [first, last] = pair, acc ->
          insertions
          |> Enum.reduce(acc, fn [pattern, insert], acc2 ->
            case pattern == pair do
              true -> [first, insert] |> Enum.reduce(acc2, &[&1 | &2])
              false -> acc2
            end
          end)
    ...
    new_template |> Enum.frequencies()

    step(new_template, insertions, count - 1)
  end
end
