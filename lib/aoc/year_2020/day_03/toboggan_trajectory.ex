defmodule Aoc.Year2020.Day03.TobogganTrajectory do
  ...
  *.#..#...#.#*.#..#...#.#.#..#...#.#.#..#...#.#.#..#...#.#.#..#...#.#  --->
  ...
  def part_1(input) do
    String.split(input, "\n")
    |> Enum.slice(1..-1)
    |> Enum.reduce({0, 0, 0, 0}, &count_trees/2)
    |> IO.inspect()
  end

  defp count_trees(row, {trees, x, y, sheet}) do
    width = String.length(row)
    maybe_x = x + 3
    new_x = rem(maybe_x, width)
    new_sheet = if new_x < x, do: sheet + 1, else: sheet
    new_trees = if String.at(row, new_x) == "#", do: trees + 1, else: trees

    debug(row, new_x)

    {new_trees, new_x, y + 1, new_sheet}
  end
